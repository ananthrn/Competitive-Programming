...
struct compare {
    bool operator() (const pll & lhs, const pll& rhs) const {
        ...
        auto lhsDiff = lhs.s - lhs.f;
        auto rhsDiff = rhs.s - rhs.f;
...
ll computeTest(const vector<ll> &a, int k){
    int n = a.size();
    
    multiset<pll, decltype(cmp)*> intervals(cmp);
    ...
    rep(val, 1, k)
    {
        ...
        auto worstPair = *(intervals.begin());
        
        
        
        if(worstPair.s - worstPair.f == 1)
        {
            return 1;
        }
        ...
        int midVal = (worstPair.f + worstPair.s)/2;
        
        pll newPairOne = {worstPair.f, midVal};
        pll newPairTwo = {midVal, worstPair.s};
        
        intervals.insert(newPairOne);
        intervals.insert(newPairTwo);
        
    }
    
    auto worstPair = *(intervals.begin());
    
    return worstPair.s - worstPair.f;
}
int main(void)
{
