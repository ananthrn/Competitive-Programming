...
int computeTest(vector<vector<int> > &plateSums, int P, int n, int k){
    // int n = plates.size();
    // int k = plates[0].size();
    ...
    repi(index, 1 , n)
    {
        repi(platesAvail, 1, P){
            // dp[index][platesUsed] = max(dp[index-1][platesUsed], plateSums[index] + dp[index-1][platesUsed-1]);
            
            dp[index][platesAvail]= 0;
            
            repi(plateIndex, 0, min(k, platesAvail)){
                
                auto thisAns = (plateSums[index][plateIndex] + dp[index-1][platesAvail - plateIndex]);
                
                dp[index][platesAvail] = max(dp[index][platesAvail], thisAns);
            }
...
int main(void)
{
    ...
    rep(testNum, 1, t){
        ...
        vector<vector<int> > plateSums(n+1, vector<int>(k+1, 0));
        ...
        rep(j, 1, n)
        {
            rep(r, 1, k)
            {
                ...
                plateSums[j][r] = plateSums[j][r-1] + plateVal;
            }
            
            // rep(r, 1, k)
            // {
            //     plateSums[j][r] = plateSums[j][r-1]
            // }
        }
        auto ans = computeTest(plateSums, P, n , k);
