...
pair<int,vi> compute_ans(int n, int m, const vector<vi>&adj)
{
    if(m%2){
...
bool verifyAns(int n, int m, const vi &ans, const vector<pii> &edges){
    int num_edges = 0;
    for(auto edge: edges){
        if(ans[edge.f-1]==ans[edge.s-1])
            num_edges+=1;
    }
    if(DEBUG)
    {
    ...
    return num_edges%2 == 0;
}
// Submission Link: https://www.codechef.com/viewsolution/26967650
int main(void)
{
    ...
    while(t--)
    {
        ...
        auto ans_test = compute_ans(n,m,adj);
        
        bool verify_ans = verifyAns(n,m, ans_test.s, edges);
        
        if(!verify_ans)
        {
            throw logic_error("Edges not even\n");
        }
        cout<<ans_test.f<<"\n";
        print(ans_test.s);
