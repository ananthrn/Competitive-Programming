...
vector<vi> computeRowManacher(const vector<vi> &arr){
    assert(!arr.empty());
    vector<vi> ans(arr.size(), vi(arr[0].size(), 0));
    // return ans;
    int n = arr.size();
    int m = arr[0].size();
    ...
    repi(r, 0, n-1){
        ...
        repi(c, 0, m-1){
            if(c <= R){
                rad = min(ans[r][2*C-c], R-c);
            } else {
                rad = 0;
            }
            
            while(c+rad<m and c-rad>=0 and arr[r][c-rad] == arr[r][c+rad]){
                rad++;
            }
...
vector<vi> computeColManacher(const vector<vi> &arr){
    assert(!arr.empty());
    ...
    repi(c, 0, m-1){
        ...
        repi(r, 0, n-1){
            if(r <= R){
                rad = min(ans[2*C-r][c], R-r);
            } else {
                rad = 0;
            }
            
            while(r+rad<n and r-rad>=0 and arr[r-rad][c] == arr[r+rad][c]){
                rad++;
            }
...
ll compute_test_case(const vector<vi> &arr){
    auto man = computeRowManacher(arr);
    auto man_col = computeColManacher(arr);
...
// Solution: https://www.codechef.com/viewsolution/28651034
int main(void)
{
