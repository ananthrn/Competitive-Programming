// Link: https://www.codechef.com/viewsolution/31665411
//#define SUBMISSION
#define BRUTE_FORCE
#ifdef SUBMISSION
#undef BRUTE_FORCE
...
int computeSimilarity(vector<int>& a, const vector<int>& b){
    ...
    {
        similarity += (a[j] == b[j])?1:0;
    }
    
    return similarity;
}
vector<int> computeTestMajority(const int& n, const int& m, const vector<vi> & answerForms){
    
    int k = answerForms[0].size();
    ...
    {   
        int bestAnswer = 0;
        int bestAnswerCount = 0;
        vector<int> answerCounts(m+1, 0);
        
        rep(formNumber, 0, k-1){
            answerCounts[answerForms[qNum][formNumber]]+=1;
            
            if(answerCounts[answerForms[qNum][formNumber]] > bestAnswerCount){
                bestAnswerCount = answerCounts[answerForms[qNum][formNumber]];
                bestAnswer = answerForms[qNum][formNumber];
            }
        }
        
        answers[qNum] = bestAnswer;
    }
    
    return answers;
}

vector<int> computeTestSequential(const int& n, const int& m, const vector<vi> & answerForms){
    
    int k = answerForms[0].size();
    
    vector<int> answers(n, 0);
    
    int rollingFormNumber = 0;
    ...
    {   
        ...
        answers[qNum] = answerForms[qNum][qNum%k];
    }
...
int main(void)
{
    ...
    rep(testNum, 1, t){
        ...
        vector<vi> answerForms(n, vi(k, 0));
        
        rep(qNum, 0, n-1)
        {   
            rep(formNumber, 0, k-1){
                cin>>answerForms[qNum][formNumber];
            }
        }
        
        // auto ansMajority = computeTestMajority(n, m, answerForms);
        auto ansSequence = computeTestSequential(n, m, answerForms);
