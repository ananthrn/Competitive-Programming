// https://www.codechef.com/viewsolution/30245913
#define SUBMISSION
#define BRUTE_FORCE
#ifdef SUBMISSION
#undef BRUTE_FORCE
...
const int DEBUG = 0;
int computeQueryBrute(const vll & a, const vll &query){
    auto x1 = query[0];
    auto x2 = query[1];
    auto y = query[2];
    ...
    rep(j, x1, x2-1){
        if(a[j]<=y and a[j+1]>=y or a[j]>=y and a[j+1]<=y)
        {   ans+=1;
        }
    }
...
vector<int> computeTestBrute(const int&n, const vll& a, const vector<vll> & quers){
    
    int quer_sz = quers.size();
    vector<int> ans(quer_sz, 0);
    
    
    rep(j, 0, quer_sz-1)
    {
        ans[j] = computeQueryBrute(a, quers[j]);
    }
...
unordered_map<ll, int> computeCompressedIndices(vll &a){
    sort(a.begin(), a.end());
    
    unordered_map<ll, int> indexMap;
...
vector<int> computeTestRange(const int&n, const vll& a, const vector<vll> & quers){
    int quer_sz = quers.size();
    vll arr(a.begin(), a.end());
    ...
    auto indexMap = computeCompressedIndices(arr);
    
    vector<vector<pair<int, pair<int, ll> > >> queriesAtPos(n+1);
    
    rep(quer_index, 0 , (int)quers.size() - 1){
        auto queryInsert = make_pair(0, make_pair(quer_index, quers[quer_index][2]));
        auto queryEnd = make_pair(1, make_pair(quer_index, quers[quer_index][2]));
        
        queriesAtPos[quers[quer_index][0]].pb(queryInsert);
        queriesAtPos[quers[quer_index][1]].pb(queryEnd);
    }
    
    vi ans(quer_sz, 0);
    
    vector<int> recordedAnswer(quer_sz+1, -1);
    FenwickTreeRangeUpdate fTree(n+quer_sz+1);
    ...
    rep(index, 1 , n){
        ...
        for(const auto &quer: queriesAtPos[index]){
            auto queryType = quer.first;
            auto queryIndex = quer.second.first;
            auto yValue = quer.second.second;
            
            if(DEBUG)
            {
                cout<<"queryType: "<<queryType<<endl;
                cout<<"queryIndex: "<<queryIndex<<endl;
                cout<<"yValue: "<<yValue<<endl;
            }
            if(queryType==0){
                assert(recordedAnswer[queryIndex]==-1);
                
                recordedAnswer[queryIndex] = fTree.query(indexMap[yValue]);
                
                if(DEBUG)
                {
                    cout<<"recordedAnswer[queryIndex]: "<<recordedAnswer[queryIndex]<<endl;
                    cout<<endl;
                }
                ...
                auto thisAns = fTree.query(indexMap[yValue]);
                auto recorded = recordedAnswer[queryIndex];
                
                if(DEBUG)
                {
                    cout<<"thisAns: "<<thisAns<<endl;
                    cout<<"recorded: "<<recorded<<endl;
                    cout<<endl;
                }
                ans[queryIndex] = thisAns - recorded;
            }
        }   
        if(index<n)
        {   
            auto minDex = min(indexMap[a[index]], indexMap[a[index+1]]);
            auto maxDex = max(indexMap[a[index]], indexMap[a[index+1]]);
            
            fTree.updateRange({minDex, maxDex}, 1);
            
            if(DEBUG){
                cout<<"updateRange: "<<a[index]<<", "<<a[index+1]<<endl;
            }
        }
...
int main(void)
{
    ...
    rep(testNum, 1, t){
        ...
        vector<vll> quers(q, vll(3, 0LL));
        
        rep(j, 0, q-1)
        {
            cin>>quers[j][0]>>quers[j][1]>>quers[j][2];
        }
        
        
        auto queryAnswersOpt = computeTestRange(n, a, quers);
        
        // auto queryAnswers = computeTestBrute(n, a, quers); 
        // if(queryAnswers!=queryAnswersOpt)
        // {
        //     throw logic_error("queryAnswers!=queryAnswersOpt");
        // }
        
        for(const auto & queryAnswer: queryAnswersOpt)
        {
            cout<<queryAnswer<<'\n';
        }
        
        // cout<<"queryAnswersOpt: ";
        // print(queryAnswersOpt);
        if(DEBUG){
            cout<<dash<<endl<<endl;
        }
    }
