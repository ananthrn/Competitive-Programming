// Solution Link: https://www.codechef.com/viewsolution/30698876
...
const int DEBUG =  0;
const auto MAX_VAL = mx5+5;
const ll modVal = 998244353LL;

auto factMod = preComputeFactorialMod(MAX_VAL, modVal);
auto inverseFactMod = preComputeInverseFactorialMod(MAX_VAL, modVal);
...
class DistinctVals{
    private:
        ...
        vector<bool> valExists;
        vector<int> countVals;
        
        unsigned int MAX_VAL = 0;
        
        void purgeDeletedVals(){
            vector<int> newDistinctVals;
            
            for(const auto& val: distinctVals){
                if(countVals[val]!=0)
                {
                   newDistinctVals.push_back(val); 
                } else 
                {
                    valExists[val] = false;
                }
        ...
        void removeVal(const int &val){
            countVals[val] = 0;
        }
    public:
    
        DistinctVals(const unsigned int & MAX_VAL){
            valExists = vector<bool> (MAX_VAL + 1, false);
            countVals = vector<int>  (MAX_VAL + 1, 0);
            
            this->MAX_VAL = MAX_VAL;
        }
        
        void resetVals(const unsigned int & MAX_VAL){
        ...
        void addCountToVal(const int &val, const int & newCount){
            if(!valExists[val])
            {
                addVal(val);
            }
            
            countVals[val]  = max(0, countVals[val] + newCount);
        }

        
        vector<int> getActiveVals(){
        ...
        unsigned int getCountForVal(const int &val)
        {
            return countVals[val];
        }
        
};

DistinctVals countOfCounts(mx5+1);

ll computeQueryBrute(const pair<pii, int> & query, const vll &a){
    unordered_map<ll, int> heapCountMap;
    
    rep(index, query.first.first, query.first.second){
        heapCountMap[a[index]]+=1;
    }
    
    vector<int> heapCounts;
    
    for(const auto & count: heapCountMap){
        heapCounts.pb(count.s);   
    }
    
    auto xorVal = 0LL;
    
    for(const auto & heapVal: heapCounts){
        xorVal ^= heapVal;
    }
    ...
    auto winningMoves = 0LL;
    
    for(const auto & heapVal: heapCounts){
        auto yVal = heapVal ^ xorVal;
        
        if(yVal < heapVal){
            

            auto numWays = combinationMod(heapVal, yVal, factMod, inverseFactMod, modVal)%modVal;
            
            
            
            winningMoves = (winningMoves + numWays)%modVal;

            if(DEBUG){
                cout<<"heapVal: "<<heapVal<<endl;
                cout<<"yVal: "<<yVal<<endl;
                cout<<"numWays: "<<numWays<<endl;
                // auto numWaysBrute = combinationMod(heapVal, yVal, modVal);
                // cout<<"numWaysBrute: "<<numWaysBrute<<endl;
                cout<<endl;
            }
        }
    }
...
vector<ll> computeTestBrute(const int &n, const vll &a, const vector<pair<pii, int> > &queries){
    
    vector<ll> queryAnswers(queries.size(), 0LL);
    
    rep(j, 0, (int)queries.size()-1){
        queryAnswers[j] = computeQueryBrute(queries[j], a);
    }
...
bool compareQuery(const pair<pii, int> &a, const pair<pii, int> &b){
    return make_pair(a.first.first/BLOCK_SIZE, a.first.second) < make_pair(b.first.first/BLOCK_SIZE, b.first.second);
}


void updateMo(const int &valToUpdate, const int &newCount, vector<int> & counts){
    
    if(DEBUG)
    {
        cout<<"STATE BEFORE UPDATE: "<<endl;
        
        auto distinctVals = countOfCounts.getActiveVals();
        vector<int> countPlus;
        
        for(const auto& distinctVal: distinctVals)
        {
            countPlus.pb(countOfCounts.getCountForVal(distinctVal));
        }
        
        cout<<"val to update: "<<valToUpdate<<endl;
        cout<<"newCount: "<<newCount<<endl;
        cout<<"counts: "; print(counts);
        cout<<"distinctCounts: "; print(distinctVals);
        cout<<"countPlus: "; print(countPlus);
        
        cout<<endl;
    }
    
    //XOR out the old value
    totalXor ^= counts[valToUpdate];
    
    countOfCounts.addCountToVal(counts[valToUpdate], -1);
    
    counts[valToUpdate] += newCount;
    countOfCounts.addCountToVal(counts[valToUpdate], 1);
    
    //XOR in the new counts
    totalXor ^= counts[valToUpdate];
...
vector<ll> computeTestMo(const int &n, const vll &a, vector<pair<pii, int> > &queries){
    //Init for new test
    
    totalXor = 0;
    countOfCounts.resetVals(n);
    ...
    vector<int> newVals(n + 1, 0);
    
    map<int, int> valToPos;
    
    for(int i=1; i<=n; ++i) {

		if(valToPos.find(a[i])==valToPos.end())
			valToPos.insert({a[i], valToPos.size() + 1});
		newVals[i]=valToPos[a[i]];
	}
    ...
    counts[newVals[1]]=1;
    totalXor = counts[newVals[1]];
    
    countOfCounts.addCountToVal(1, 1);
    
    
    if(DEBUG)
    {
        cout<<"newVals: "; print(newVals);
        cout<<endl;
    }
    for(const auto & query: queries){
        ...
        if(DEBUG)
        {
            cout<<"query: "<<"("<<query.first.first<<", "<<query.first.second<<"), index: "<<query.second<<endl;
            // cout<<"currentL: "<<currentL<<endl;
            // cout<<"currentR: "<<currentR<<endl;
            cout<<endl;
        }
        while(query.first.first < currentL)
        {   
            currentL -= 1;
            updateMo(newVals[currentL], 1, counts);

        }
        
        while(query.first.second > currentR)
        {   
            currentR+=1;
            updateMo(newVals[currentR], 1, counts);
        }
        ...
        auto distinctCounts = countOfCounts.getActiveVals();
        
        if(DEBUG)
        {
            cout<<"finalDistinctCounts: ";
            print(distinctCounts);
            cout<<endl;
        }
        if(totalXor!=0)
        {   for(const auto &distCount : distinctCounts)
            {
                if((distCount ^ totalXor) < distCount){
                    int newVal = distCount ^ totalXor;
                    ll totalCount = countOfCounts.getCountForVal(distCount);
                    ...
                    ll thisMoves = combinationMod(distCount, newVal, factMod, inverseFactMod, modVal)%modVal;
                
                    ll totalMoves = (totalCount * thisMoves)%modVal;
                
                    winningMoves = (winningMoves + totalMoves)%modVal;
                }
            }
        
            queryAnswers[query.s] = winningMoves;
        } else{
            queryAnswers[query.s] = 0LL;
        }
...
int main(void)
{
    ...
    rep(testNum, 1, t){
        ...
        rep(j, 0, q-1){
            cin>>queries[j].f.f>>queries[j].f.s;
            queries[j].s = j;
        }
        
        // auto queryAnswers = computeTestBrute(n, a, queries);
        
        auto queryAnsMo = computeTestMo(n, a, queries);
        
        // if(queryAnswers!= queryAnsMo){
        //     throw logic_error("queryAnswers!= queryAnsMo");
        // }
        for(const auto & queryAns: queryAnsMo){
            cout<<queryAns<<'\n';
        }
        if(DEBUG){
                cout<<dash<<endl<<endl;
