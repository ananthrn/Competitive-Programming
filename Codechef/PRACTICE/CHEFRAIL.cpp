...
const int DEBUG = 0;
ll computeXOpt(const vll &xVec, const vll &yVec){
    ll n = xVec.size();
    ll m = yVec.size();
    
    // unordered_set<int> xNegativeSet;
    // unordered_set<int> xPositiveSet;
    
    // unordered_set<int> ySet(yVec.begin(), yVec.end());
    
    vector<bool> xNeg(mx5+1, false);
    vector<bool> xPos(mx5 +1 ,false);
    
    vector<int> yAll(mx5+1, 0);
    ...
    for(const auto &xval: xVec)
    {
        if(xval >0)
        {
        //   xPositiveSet.insert(xval);  
          xPos[xval] = true;
        } else if(xval<0)
        {
            // xNegativeSet.insert(xval);
            xNeg[-xval] = true;
        } else if(xval==0){
            checkZero = true;
        }
    ...
    for(const auto &yVal: yVec){
        yAll[abs(yVal)] +=1;
    }
    
    ll ans = 0;
    ll secondAns = (checkZero)?(n-1)*(m):0;
    ...
    while(a * a <= mx5){
        ...
        while(b*b <= mx5){
            if(greatestCommonDivisor(a, b) == 1){
                // a and b have to be co-prime according to our assumption
                ll g = 1;
                while(g * (max(a, b) * max(a, b)) <= mx5)
                {
                    // if(xNegativeSet.count(- g * a * a)!=0 and  xPositiveSet.count(g*b*b)!=0){
                    //     ans+=ySet.count(g*a*b) + ySet.count(-g*a*b);
                    // }
                    
                    if(xNeg[g*a*a] and  xPos[g*b*b])
                    {
                        ans += yAll[g*a*b];
                    }
                    g+=1;
    ...
    return ans + secondAns;
}
ll computeX(const vll& xVec, const vll& yVec){
    ll n = xVec.size();
    ll m = yVec.size();
    
    unordered_set<int> xSet(xVec.begin(), xVec.end());
    ...
    for(auto y: yVec){
        for(auto x: xVec){
            ...
            if(x!=0  and ((y*y) % x ==0) and (xSet.count(-(y*y)/x)!=0)){
                ans+=1;
            }
            
            if(DEBUG){
                cout<<"x, y: "<<x<<" "<<y<<endl;
                cout<<"-(y*y)/x: "<<-(y*y)/x<<endl;
                cout<<"ans: "<<ans<<endl;
                cout<<endl;
            }
        }
    ...
    ll secondAns = (xSet.count(0))?(n-1)*(m):0;
    return ans/2 + secondAns;
}
ll computeAnswer(const vll &xVec, const vll &yVec){
    return computeXOpt(xVec, yVec) + computeXOpt(yVec, xVec);
}
// Solution Link: https://www.codechef.com/viewsolution/29798369
int main(void)
{
    ...
    while(t--){
        ...
        auto ans = computeAnswer(xVec, yVec);
