...
class SegmentTreeNode{
    ...
    ll total;
    bool isFlippedUpdate;
    
   
    public:
    int start,end;
    
    SegmentTreeNode():total(0), start(-1), end(-1),isFlippedUpdate(false){} 
    
    void assignLeaf(int value, int start, int end){
        total = value;
        this->start = start;
        this->end = end;
    }
    void merge(SegmentTreeNode &left, SegmentTreeNode&right){
        ...
        ll leftTotal = (left.isFlippedUpdate)?left.getFlippedValue():left.getValue();
        ll rightTotal = (right.isFlippedUpdate)?right.getFlippedValue():right.getValue();
    ...
    bool ifPendingUpdate(){
        return isFlippedUpdate;
    }
    
    void applyPendingUpdate(){
        this->total = this->getFlippedValue();
        isFlippedUpdate = false;
    }
    
    void toggleFlipUpdate(){
        this->isFlippedUpdate = !this->isFlippedUpdate;
    }
};
...
class SegmentTree{
    ...
    // }
     ...
     void updateRange(int startPos, int endPos){
         //flip all coins in [startPos,endPos]
         updateRange(1,startPos,endPos);
     }
    ...
    void updateRange(int stIndex, int startPos, int endPos){
        if(startPos == nodes[stIndex].start and endPos == nodes[stIndex].end){
            nodes[stIndex].toggleFlipUpdate();
            return;
        }
        ...
        if(endPos<=mid){
            updateRange(leftChildIndex,startPos,endPos);
        }else if(startPos>mid){
            updateRange(rightChildIndex,startPos,endPos);
        }else{
            updateRange(leftChildIndex,startPos,mid);
            updateRange(rightChildIndex,mid+1,endPos);
        }
...
// Soln Link: https://www.codechef.com/viewsolution/22605305
int main(void)
{
    ...
    rep(j,1,m){
        ...
        if(op == 0){
            //cout<<"j op x y:"<<j<<" "<<op<<" "<<x<<" "<<y<<"\n";
            seg.updateRange(x,y);
        } else{
            //cout<<"j op x y:"<<j<<" "<<op<<" "<<x<<" "<<y<<"\n";
            cout<<seg.getValue(x,y)<<"\n";
        }
    }
    return 0;
}
