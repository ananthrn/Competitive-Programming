...
void preComputeTrieNodeVec(int root, int par, 
    vector< set< TrieXORSetNode *> > & allowedTrieNodesVec, 
    const vector<vi> &adj) {
    }
ll compute_ans(){
...
bool computeAnsBruteForce(string s, int flipDirection=0){
    int n = s.length();
    ...
    if(flipIndex!=-1)
        s[flipIndex]=(s[flipIndex]=='0')?'1':'0';
        
    // cout<<"s: "<<s<<"\n";
    // cout<<"flipDirection: "<<flipDirection<<"\n";
    if(dp.count(s))
        return dp[s];    
    ...
    rep(i, 0, n-1){
        if(s[i]=='1'){
            // cout<<"s: "<<s<<" branching at "<<i<<"\n";
            string leftString = s.substr(0,i);
            bool leftAns = computeAnsBruteForce(leftString, -1);
            if(!leftAns)
                continue;
                
            string rightString = (i<n-1)?s.substr(i+1):"";
            
            bool rightAns = computeAnsBruteForce(rightString,1);
            bool ans = leftAns and rightAns;
            if(ans){
                dp[s]=true;
                return true;
            }
...
bool computeAnsFast(const string &s){
    int numOnes = 0;
    for(char c:s){
        numOnes+=(c=='1');
    }
...
bool runTest(int highestNum = 100)
{
    const int size = 20;
    rep(num,0,highestNum){
        string s = bitset<size>(num).to_string();
        bool testSame = computeAnsBruteForce(s) == computeAnsFast(s);
        if(!testSame){
            string outp = "Wrong answer for " +s;
            throw logic_error(outp);
        }
    }
}
// Soln Link: https://www.codechef.com/viewsolution/25961843
int main(void)
{
